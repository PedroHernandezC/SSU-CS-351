...
void julia(Complex d, Complex center, Color* pixels) {
	...
	const int idx = iy * (int)Width + ix;
	const float x = (ix - (float)Width  * 0.5f) * d.x + center.x;
    	const float y = (iy - (float)Height * 0.5f) * d.y + center.y;
    	Complex z(x, y);

	const Complex c(-0.8f, 0.156f);

	int iter = 0;
   	while (iter < (int)MaxIterations && magnitude(z) <= 2.0f) {
        	z = z * z + c;
        	++iter;
    	}
